package io.demor.nuts.lib.viewmapping;
...
public final class ViewMapUtil {
    ...
    public static View map(Object object, LayoutInflater inflater,
                           ViewGroup root) {
        ViewMapping viewMapping = getViewMapping(object.getClass());
        if (viewMapping == null) {
            return null;
        }
        View rootView = inflater.inflate(viewMapping.value(), root, false);
        map(object, rootView);
        return rootView;
    }
    ...
    public static <T> Pair<T, View> map(Class<T> clazz,
        ...
        try {
            ...
            ViewMapping viewMapping = getViewMapping(clazz);
            if (viewMapping == null) {
                return pair;
            }
            View rootView = inflater.inflate(viewMapping.value(),
                    root, false);
