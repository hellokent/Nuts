...
* 有些耗时调用在前台UI调用时，需要一个生命周期回调，来做调用前后`Dialog`的显示和消失
...
这个就是Nuts中间件（或称为Controller）的核心特性。
...
需要支持同步异步调用的函数，需要用`Return`对象封装一下函数返回值，对于void类型的返回值，使用VoidReturn对象。
...
返回值需要被`Return`对象包装一次再返回出来，示例如下：
...
调用业务实例里面的调用方法，返回的其实是`Return`对象，并不是调用方需要的返回值，需要知道同步异步方法之后，才能获取到真正的返回值。

同步调用，就是阻塞当前线程，直到方法在线程池中执行结束，最后将执行结果传至当前线程。

异步调用，就是不阻塞UI线程，方法执行接收后，将方法的执行结果通知给预先注册好的`Callback`。
...
在很多时候，调用前后会显示一个`Dialog`，可以使用`asyncUIWithDialog`方法。

`asyncUIWithDialog`这个方法需要传入一个`dialog`参数，用来通知Nuts在方法调用前后需要展示和消失哪个Dialog。
...
同步调用很简单，`sync`方法：`ACCOUNT_CONTROLLER.logout().sync();`
...
在`Activity`、`Fragment`或者自定义View里面调用业务逻辑的时候，假如需要在当前Activity被Destroy之前，结束当前所有调用，可以使用`checkActivity`属性。

checkActivity的作用是：设置在回调和业务逻辑方法执行前，检查Activity是否存活，假如没有存活，则直接返回，没有下一步的执行。
...
2. Return对象的调用链里设置checkActivity
...
抛异常的时候，需要用`ExceptionWrapper`包装过后才能抛出，在接口和实现的方法声明处不需要用`throws`声明异常。

当是异步调用时，需要实现传入的`ControllerListener`里面有`handleException`的方法：
